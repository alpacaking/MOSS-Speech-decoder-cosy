...
class GLM4Codec(nn.Module):
    ...
    def __init__(self):
        ...
        self.audio_decoder = AudioDecoder(
            ...
            campplus_model='/inspire/hdd/project/embodied-multimodality/public/lzjjin/Streaming-Codec/flow/campplus.onnx',
        )
    ...
    @torch.inference_mode()
    def decode(self, codes_list: List[torch.Tensor],speech_token=torch.zeros(1, 0, dtype=torch.int32),
                  speech_feat=torch.zeros(1, 0, 80), device: torch.device = torch.device("cuda")) -> Dict[str, Any]:
        """
        Args:
        ...
        for codes in codes_list:  # codes: (1, T)
            ...
            prompt_speech_feat = speech_feat.to(device)  # (1, 0, 80)
            flow_prompt_speech_token = speech_token.to(device)  # (1, 0)
            codes=torch.tensor(codes).unsqueeze(0)
            # 解码

            tts_speech, _ = self.audio_decoder.stream_inference( # ???
                codes,  # (1, T)
...
if __name__=='__main__':
    encoder=GLM4Codec().to('cuda')
    result1=encoder.encode_token('/inspire/hdd/project/embodied-multimodality/public/lzjjin/todecode.wav')
    # result2=encoder.encode_token_test('/inspire/hdd/project/embodied-multimodality/public/datasets/haitianruisheng_3/segment_00_000/00460fd0bd40915bd31e79976176c9b5_001.mp3')
    # print(result1)
    # print(result2)
    wav=encoder.decode([result1])
    torchaudio.save('result1.wav',wav['syn_wav_list'][0].unsqueeze(0).cpu(),sample_rate=24000)
    
    wav=encoder.decode_streaming([result1],prompt_speech='/inspire/hdd/project/embodied-multimodality/public/datasets/LibriTTS/LibriTTS/train-clean-100/103/1241/103_1241_000027_000007.wav')
