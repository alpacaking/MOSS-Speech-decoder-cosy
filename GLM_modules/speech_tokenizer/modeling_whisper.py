...
class CausalConv1d(nn.Conv1d):
    ...
    def forward_causal(self, inp: torch.Tensor, conv_cache: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Performs a causal convolution with streaming capabilities.
        Args:
            inp (`torch.Tensor`): The current input chunk of shape `(batch_size, in_channels, seq_len)`.
            conv_cache (`torch.Tensor`, *optional*): The cached right-most part of the previous input chunk,
                                                     used for left padding. Shape `(batch_size, in_channels, left_padding)`.
        Returns:
            `Tuple[torch.Tensor, torch.Tensor]`:
                - `output`: The convolved output of shape `(batch_size, out_channels, new_seq_len)`.
                - `new_conv_cache`: The new cache, containing the right-most part of the current input chunk
                                  needed for the next `forward_causal` call.
        """
        batch_size, in_channels, seq_len = inp.shape
        if conv_cache is None:
            padded_inp = torch.nn.functional.pad(inp, (self.left_padding, 0))
        else:
            padded_inp = torch.cat([conv_cache, inp], dim=2)

        output = super(CausalConv1d, self).forward(padded_inp)
        new_conv_cache = inp[:, :, -self.left_padding:]
        if new_conv_cache.shape[2] < self.left_padding:
            new_conv_cache = torch.nn.functional.pad(new_conv_cache, (self.left_padding - new_conv_cache.shape[2], 0))
...
class WhisperSdpaAttention(WhisperAttention):
    def forward(
        ...
        sign=False
        if is_causal==True and cache_position is not None:
            dtype, device = query_states.dtype, query_states.device
            min_dtype = torch.finfo(dtype).min
            causal_mask=_prepare_4d_causal_attention_mask_with_cache_position(attention_mask=None,sequence_length=query_states.shape[-2],target_length=key_states.shape[-2],cache_position=cache_position,dtype=dtype,device=device,min_dtype=min_dtype,batch_size=query_states.shape[0])
        elif cache_position is None and is_causal==True:
            sign=True
...
class WhisperVQEncoderLayer(nn.Module):
    def __init__(self, config: WhisperVQConfig, is_causal=True, layer_idx=None):
        ...
        self.self_attn = WHISPER_ATTENTION_CLASSES[config._attn_implementation](
            ...
            config=config,
            is_causal=is_causal,
            layer_idx=layer_idx if layer_idx is not None else None,
        )
        self.is_causal = is_causal
        if self.is_causal:
    ...
    def forward_causal(
            ...
            layer_head_mask: torch.Tensor,
            output_attentions: bool = False,
            past_key_value = None, # 新增参数，用于接收self-attention的cache
            cache_position: Optional[torch.LongTensor] = None, # 新增参数，用于指示当前块在完整序列中的起始位置
    ) -> torch.Tensor:
        """
        Args:
            hidden_states (`torch.FloatTensor`): input to the layer of shape `(batch, seq_len, embed_dim)`
        ...
        hidden_states, attn_weights, updated_past_key_value = self.self_attn(
            hidden_states=hidden_states,
            attention_mask=attention_mask if not self.is_causal else None, # 如果是因果注意力，外部的attention_mask通常不用于因果性
            layer_head_mask=layer_head_mask,
            output_attentions=output_attentions,
            past_key_value=past_key_value, # 传递给self_attn
            cache_position=cache_position # 传递给self_attn
        )
        hidden_states = nn.functional.dropout(hidden_states, p=self.dropout, training=self.training)
        hidden_states = residual + hidden_states
        ...
        return outputs,updated_past_key_value
...
class WhisperVQEncoder(WhisperPreTrainedModel):
    ...
    def __init__(self, config: WhisperVQConfig):
        ...
        if config.quantize_encoder_only:
            ...
            self.layers = nn.ModuleList([WhisperVQEncoderLayer(config, is_causal=config.encoder_causal_attention or (
                        config.quantize_causal_encoder and layer_id < config.quantize_position),layer_idx=layer_id) for layer_id in
    ...
    #     """Resets the causal cache for a new sequence."""
    #     self._causal_hidden_states_cache = None
    #     self._causal_attention_mask_cache = None
    #     self._causal_current_sequence_length = torch.tensor(0, dtype=torch.long, device=self.device)
    ...
    def forward_causal(
        self,
        input_features: torch.FloatTensor,  # 传入当前L_chunk的特征
        attention_mask: Optional[torch.LongTensor] = None, 
        past_key_values: Optional[EncoderDecoderCache] = None, # 传递EncoderDecoderCache
        conv1_cache = None,
        conv2_cache = None,
        ...
        cache_position: Optional[torch.LongTensor] = None, # 用于指示当前块在完整序列中的起始位置
        quantized_token_ids: Optional[torch.LongTensor] = None,
    ):
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (
        ...
        batch_size, feature_size, chunk_length = input_features.shape
        assert batch_size==1
        current_seq_length = chunk_length // (self.conv1.stride[0] * self.conv2.stride[0])

        past_key_values_length = 0
        if past_key_values is not None and past_key_values.self_attention_cache is not None:
            past_key_values_length = past_key_values.self_attention_cache.get_seq_length()

        if cache_position is None:
            cache_position = torch.arange(
                past_key_values_length, past_key_values_length + current_seq_length, device=input_features.device
            )
        if attention_mask is not None:
            current_attention_mask = attention_mask[:, :: (self.conv1.stride[0] * self.conv2.stride[0])]
            if self.config.quantize_causal_block_size is not None:
                extended_attention_mask = current_attention_mask
            else:
                extended_attention_mask = current_attention_mask
        else:
            extended_attention_mask = None
        # conv1_output, new_conv1_cache = self.conv1.forward_causal(input_features, self._conv1_cache)
        ...
        if isinstance(self.conv2, CausalConv1d):
            conv2_output, new_conv2_cache = self.conv2.forward_causal(inputs_embeds, conv2_cache)
        else: 
            conv2_output = self.conv2(inputs_embeds)

        inputs_embeds = nn.functional.gelu(conv2_output)
        inputs_embeds = inputs_embeds.permute(0, 2, 1) # (batch, chunk_seq_len, embed_dim)


        # 6. 位置编码
        # 位置编码需要根据 cache_position 来取，以确保正确的位置信息
        embed_pos = self.embed_positions.weight
        hidden_states = inputs_embeds + embed_pos[cache_position]
        hidden_states = nn.functional.dropout(hidden_states, p=self.dropout, training=self.training)
        ...
        # 7. Transformer 层处理
        # 注意：这里需要传入 past_key_values 给EncoderLayer，以便其内部更新
        if past_key_values is None:
            # 如果没有传入past_key_values，则初始化一个BaseCache作为self_attention_cache
        ...
        for idx, encoder_layer in enumerate(self.layers):
            ...
            if to_drop:
                ...
                # 关键：EncoderLayer需要更新其self_attn_cache
                # past_key_value在encoder_layer中会被更新
                # 注意力层的 attention_mask 在因果模式下通常传入 None，让内部处理
                layer_outputs,_ = encoder_layer.forward_causal(
                    hidden_states,
                    attention_mask=extended_attention_mask, # 或者直接传入None，依赖is_causal
                    layer_head_mask=(head_mask[idx] if head_mask is not None else None),
                    output_attentions=output_attentions,
                    past_key_value=past_key_values if past_key_values is not None else None,
                    cache_position=cache_position # 传递cache_position
                )
                hidden_states = layer_outputs[0]
        ...
        return QuantizedBaseModelOutputWithCache(
            last_hidden_state=hidden_states, hidden_states=encoder_states, attentions=all_attentions,
            quantized_token_ids=quantized_token_ids, past_key_value=past_key_values,conv1_cache=new_conv1_cache,conv2_cache=new_conv2_cache
        )
