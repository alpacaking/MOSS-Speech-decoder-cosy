...
def process_gradio_audio(audio_data):
    """
    ...
    Returns:
        torch.Tensor: 处理后的音频张量 [channels, samples]
    """
...
def process_audio_nonstreaming(
    ...
    try:
        ...
        # 处理输入音频
        print("\n[STEP 1] 处理输入音频...")
        input_tensor, input_sr = process_gradio_audio(input_audio)
        ...
        # 处理参考音频
        print("\n[STEP 2] 处理参考音频...")
        ref_tensor, ref_sr = process_gradio_audio(reference_audio)
...
def create_ui():
    ...
    with gr.Blocks(title="Streaming Codec 变声器测试", theme=gr.themes.Soft()) as demo:
        ...
        nonstream_button.click(
            ...
            inputs=[input_audio, reference_audio, use_spk_embedding, use_prompt_speech, mel_cache_len],
            outputs=[nonstream_output, nonstream_info, nonstream_state]
        )

        stream_button.click(
            fn=process_audio_streaming,
            inputs=[input_audio, reference_audio, use_spk_embedding, use_prompt_speech, block_size, max_token_len, mel_cache_len],
